...
public class RPacket extends Packet {
    public RPacket(DatagramPacket packet) {
        super(packet); //packet type for write uses the RRQWRQPacket class
    }

    public RPacket (String fileName, InetAddress address, int port) {
    	super(fileName,address,port);
    }

    public RPacket(byte[] mode, byte[] filename, InetAddress address, int port) {
